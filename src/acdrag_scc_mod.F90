...
CONTAINS
  ...
  & PRCORI, PGETRL, PGWDCS, PVRLAN, PVRLDI, PSTRDU, PSTRDV, PRAPTRAJ)
    ...
    & ZV(KLON, KLEV), ZWGHT(KLON, 0:KLEV), ZDS1, ZDS2, ZFACS, ZOBST, ZPR1, ZPR2, ZREF, ZSTM, ZSTS, ZSTUS, ZSTVS, ZSUM, ZSUMD,  &
    & ZSUMF, ZSUMU, ZSUMV, ZSUSR, ZTST1, ZTST2, ZUSR, ZUSUR, ZVSUR, ZWGHTK, ZZAA, ZZBB, ZZCR, ZZCU, ZZCV
    
    INTEGER(KIND=JPIM) :: ITOP, JLEV, JLON
    
    REAL(KIND=JPRB) :: ZA, ZALP1, ZALP2, ZALPHA, ZALTI, ZARGLI, ZAUSR, ZBETA, ZD, ZDEL1, ZDEL2, ZDIFSR, ZDIFSU, ZDIFSV, ZDU,  &
    & ZDV, ZDX, ZEPS1, ZEPS2, ZEPS3, ZEPS4, ZEPS5, ZEPS6, ZFORCA, ZFORCB, ZFORCL, ZGDT, ZGDTI, ZLIFT, ZLIFTG, ZPBL, ZPBLF,  &
...
!$acc loop seq
      DO JLEV=KLEV - 1,KTDIA,-1
        ...
        IF (ZTEST /= 0.0_JPRB) THEN
          ...
          ZWGHT(JLON, JLEV) =  &
          & MAX(0.0_JPRB, (PAPRSF(JLON, JLEV + 1) - MAX(PAPRSF(JLON, JLEV), PAPRS(JLON, KLEV) - ZOBST)) / ZOBST)
...
!$acc loop seq
      ...
      ZCOS = COS(2.0_JPRB*PVRLDI(JLON))
      ZUSTAR = ZSUMU*ZCOS + ZSUMV*SIN(2.0_JPRB*PVRLDI(JLON))
      ZVSTAR = ZSUMU*SIN(2.0_JPRB*PVRLDI(JLON)) - ZSUMV*ZCOS
#else
...
!$acc loop seq
      DO JLEV=KLEV - 1,KTDIA,-1
        ...
        !     & -ZNFNO(JLON,JLEV))))
        IF (ZNFNO(JLON, JLEV) <= 0.0_JPRB)         ZTST2 = 0.0_JPRB
...
!$acc loop seq
      DO JLEV=ITOP + 1,KLEV
        ZSUMD = ZSUMD + 0.5_JPRB*(ZWGHT(JLON, JLEV) + ZWGHT(JLON, JLEV - 1))*ZIPOI(JLON, JLEV)*(ZRAPP(JLON, JLEV) - ZRAPP(JLON,  &
        & JLEV - 1))
      END DO
