SUBROUTINE ACDRAG (YDCST, YDML_PHY_MF, KIDIA, KFDIA, KLON, KTDIA, KLEV, PAPRS, PAPRSF, PDELP, PNBVNO, PRDELP, PU, PV, PRCORI,  &
& PGETRL, PGWDCS, PVRLAN, PVRLDI, PSTRDU, PSTRDV, PRAPTRAJ)
  !-----------------------------------------------------------------------
  ! - INPUT  2D .
  ! - INPUT  1D .
  ! - OUTPUT 2D .
  ...
  REAL(KIND=JPRB) :: ZIPOI(KLON, KLEV), ZNFNO(KLON, 0:KLEV), ZPOID(KLON, KLEV), ZRAPP(KLON, 0:KLEV), ZU(KLON, KLEV),  &
  & ZV(KLON, KLEV), ZWGHT(KLON, 0:KLEV), ZDS1(KLON), ZDS2(KLON), ZFACS(KLON), ZOBST(KLON), ZPR1(KLON), ZPR2(KLON), ZREF(KLON),  &
  & ZSTM(KLON), ZSTS(KLON), ZSTUS(KLON), ZSTVS(KLON), ZSUM(KLON), ZSUMD(KLON), ZSUMF(KLON), ZSUMU(KLON), ZSUMV(KLON),  &
  & ZSUSR(KLON), ZTST1(KLON), ZTST2(KLON), ZUSR(KLON), ZUSUR(KLON), ZVSUR(KLON), ZWGHTK(KLON), ZZAA(KLON), ZZBB(KLON),  &
  & ZZCR(KLON), ZZCU(KLON), ZZCV(KLON)
  
  INTEGER(KIND=JPIM) :: ITOP, JLEV, JLON
  
  REAL(KIND=JPRB) :: ZA, ZALP1, ZALP2, ZALPHA, ZALTI, ZARGLI, ZAUSR, ZBETA, ZD, ZDEL1, ZDEL2, ZDIFSR, ZDIFSU, ZDIFSV, ZDU, ZDV,  &
  & ZDX, ZEPS1, ZEPS2, ZEPS3, ZEPS4, ZEPS5, ZEPS6, ZFORCA, ZFORCB, ZFORCL, ZGDT, ZGDTI, ZLIFT, ZLIFTG, ZPBL, ZPBLF, ZPRRAP, ZRZ,  &
  ...
  & GWDAMP=>YDML_PHY_MF%YRPHY0%GWDAMP, GWDSE=>YDML_PHY_MF%YRPHY0%GWDSE, TSPHY=>YDML_PHY_MF%YRPHY2%TSPHY,  &
  & NPHYREP=>YDML_PHY_MF%YRPHY%NPHYREP, LNEWD=>YDML_PHY_MF%YRPHY%LNEWD, LGLT=>YDML_PHY_MF%YRPHY%LGLT)
  !-----------------------------------------------------------------------
  
  
  PRAPTRAJ(KIDIA:KFDIA, :) = 0.0
  !*
  !     ------------------------------------------------------------------
  ...
  DO JLON=KIDIA,KFDIA
    ZOBST(JLON) =  &
    & MIN(PAPRS(JLON, KLEV) - PAPRSF(JLON, KTDIA), MAX(PAPRS(JLON, KLEV) - PAPRSF(JLON, KLEV), HOBST*PGETRL(JLON)*PGWDCS(JLON)))
  ...
  DO JLEV=KLEV - 1,KTDIA,-1
    ...
    IF (ZTEST /= 0.0_JPRB) THEN
      ...
      DO JLON=KIDIA,KFDIA
        ZWGHT(JLON, JLEV) =  &
        & MAX(0.0_JPRB, (PAPRSF(JLON, JLEV + 1) - MAX(PAPRSF(JLON, JLEV), PAPRS(JLON, KLEV) - ZOBST(JLON))) / ZOBST(JLON))
        ZALTI = MIN(1.0_JPRB, (PAPRS(JLON, KLEV) - PAPRS(JLON, JLEV)) / ZOBST(JLON))
        ZWGHT(JLON, JLEV) = ZWGHT(JLON, JLEV)*(1.0_JPRB - ZBETA*((1.0_JPRB - ZALTI) / (1.0_JPRB + GWDPROF*ZALTI)))
  ...
  DO JLEV=KLEV - 1,KTDIA,-1
    DO JLON=KIDIA,KFDIA
      ...
      ZNFNO(JLON, JLEV) = SQRT(MAX(0.0_JPRB, PNBVNO(JLON, JLEV) + ZPBL*(ZSUMF(JLON) - PNBVNO(JLON, JLEV))))
  ...
  DO JLON=KIDIA,KFDIA
...
#endif
    ZA = PVRLAN(JLON)**2 + 0.5_JPRB*(4._JPRB*(1.0_JPRB - PVRLAN(JLON))*(1.0_JPRB + ZALP1*PVRLAN(JLON)) + PVRLAN(JLON) &
    & *LOG(1.0_JPRB / MAX(ZEPS1, PVRLAN(JLON)))*(1.0_JPRB + ZALP2*PVRLAN(JLON))) / YDCST%RPI
    ...
    ZTEMPF(JLON) = ZUSR(JLON) / MAX(ZEPS5, ZTUNE*HOBST*PGETRL(JLON)*PGWDCS(JLON)*ZNFNO(JLON, KLEV))
  ...
  DO JLEV=KLEV - 1,KTDIA,-1
    DO JLON=KIDIA,KFDIA
      ...
      ZAUSR = 0.5_JPRB*((ZU(JLON, JLEV) + ZU(JLON, JLEV + 1))*ZUSUR(JLON) + (ZV(JLON, JLEV) + ZV(JLON, JLEV + 1))*ZVSUR(JLON))
      ...
      ZSUM(JLON) =  &
      & ZSUM(JLON) + ZTST1(JLON)*(PAPRSF(JLON, JLEV + 1) - PAPRSF(JLON, JLEV))*ZNFNO(JLON, JLEV)*ZUSR(JLON) / MAX(ZEPS2, ZAUSR)
      ...
      !     & -ZNFNO(JLON,JLEV))))
      IF (ZNFNO(JLON, JLEV) <= 0.0_JPRB)       ZTST2(JLON) = 0.0_JPRB
  ...
  DO JLON=KIDIA,KFDIA
    ...
    ZSTM(JLON) = 1.0_JPRB / SQRT(1.0_JPRB + MAX(0.0_JPRB, SIGN(1.0_JPRB, ZPR1(JLON) - ZPR2(JLON) - ZEPS4)) &
    & *GWDAMP*(2.0_JPRB*SIN(MIN(ZARGLI, ZSUM(JLON))) + GWDAMP))
  ...
  DO JLEV=KTDIA,KLEV
    DO JLON=KIDIA,KFDIA
      ZRAPP(JLON, JLEV) = ZDS2(JLON)*MAX(0.0_JPRB, PAPRS(JLON, JLEV) - ZPR2(JLON)) + MAX(0.0_JPRB, MIN(ZSTM(JLON), ZRAPP(JLON,  &
      & JLEV) + ZDS1(JLON)*MAX(0.0_JPRB, PAPRS(JLON, JLEV) - ZPR1(JLON))) - ZSTS(JLON))
      ZRZ = (PAPRS(JLON, KLEV) - PAPRS(JLON, JLEV)) / MAX(ZEPS4, ZZBB(JLON)*HOBST*PGETRL(JLON)*PGWDCS(JLON))
      ...
      ZRAPP(JLON, JLEV) = ZRAPP(JLON, JLEV) + ZZCR(JLON)*(ZZAA(JLON)*SQRT(MAX(0.0_JPRB, (1.0_JPRB - ZRZ*ZRZR)**3 / (1.0_JPRB +  &
      & ZRZ*GWDPROF*ZZBB(JLON)))))
    END DO
  END DO
  ...
  DO JLEV=KTDIA,KLEV
    ...
    DO JLON=KIDIA,KFDIA
      ...
      ZRAPP(JLON, JLEV) = ZRAPP(JLON, JLEV - 1) + ZDIFSR*(1.0_JPRB + ZLIFTG*(1.0_JPRB - ZALTI) / (1.0_JPRB + ZALTI*GWDPROF)) &
  ...
  DO JLON=KIDIA,KFDIA
    ZSUMD(JLON) = 0.5_JPRB*ZWGHT(JLON, ITOP)*ZIPOI(JLON, ITOP)*(ZRAPP(JLON, ITOP) - ZRAPP(JLON, ITOP - 1))
  END DO
  
  DO JLEV=ITOP + 1,KLEV
    DO JLON=KIDIA,KFDIA
      ZSUMD(JLON) = ZSUMD(JLON) + 0.5_JPRB*(ZWGHT(JLON, JLEV) + ZWGHT(JLON, JLEV - 1))*ZIPOI(JLON, JLEV)*(ZRAPP(JLON, JLEV) -  &
      & ZRAPP(JLON, JLEV - 1))
    END DO
  END DO
  
  DO JLON=KIDIA,KFDIA
    ZSUMD(JLON) = ZSUMD(JLON) + 0.5_JPRB*ZWGHT(JLON, KLEV)*ZIPOI(JLON, KLEV)*(ZRAPP(JLON, KLEV) - ZRAPP(JLON, KLEV - 1))
  END DO
